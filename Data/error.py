...
def comparison(meanbl, confbl, meanbg, confbg, meanun, confun, meancon, confcon):
    ...
    fig, ax = plt.subplots()
    ax.bar(mult - 1.5 * width, meanbl, width, color='dimgrey', yerr=confbl, label='Exploration 5km, '
                                                                                        'Exploitation 15km', alpha=0.8)
    ax.bar(mult - 0.5 * width, meanbg, width, color='mediumvioletred', yerr=confbg, label='Exploration 10km, Exploitation 10km', alpha=1)
    ax.bar(mult + 0.5 * width, meanun, width, color='darkgoldenrod', yerr=confun, label='Exploration 15km, Exploitation 5km', alpha=1)
    ax.bar(mult + 1.5 * width, meancon, width, color='teal', yerr=confcon, label='Only Exploration (Exploration 20km)',
           alpha=0.9)
